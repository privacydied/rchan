...
var thumbSize;
var thumbAssetVersion;   // rchan: cache-busting query for thumb <img src> (see getImgTag)
var latestLimit;
...
exports.getImgTag = function(file) {
  ...
  // rchan: cache-bust the thumb URL server-side. Regenerating a thumbnail in
  // place (e.g. at a higher resolution) never changes file.thumb's path
  // (it's addressed by the ORIGINAL file's hash, not the thumb's own bytes),
  // so a browser that already cached the old bytes at that exact URL keeps
  // serving them until ITS OWN locally-recorded expiry -- a header change on
  // the server cannot retroactively shorten an already-stored cache entry,
  // and neither can a Cloudflare purge (that only clears the edge, not
  // clients). The URL must actually change for a stale local copy to ever be
  // evicted. Bump thumbAssetVersion in settings/general.json after any mass
  // thumbnail regen. Only real hash-based thumb paths get the query (matches
  // the engine's own file.thumb.length > 71 check below, which distinguishes
  // a real thumb from a short static placeholder like /genericThumb.png).
  var thumbSrc = file.thumb;
  if (thumbSrc && thumbSrc.length > 71) { thumbSrc += (thumbSrc.indexOf('?') === -1 ? '?' : '&') + 'v=' + thumbAssetVersion; }

  var img = '<img loading="lazy" src="' + thumbSrc + '"';
