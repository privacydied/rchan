...
// rchan: cache-bust thumb URLs emitted into JSON (catalog.json's client-side
// renderer reads `thumb` straight from here and sets <img src> from it
// directly -- same reasoning as domManipulator/common.js's getImgTag: the
// URL never changes when a thumbnail is regenerated in place, so a stale
// browser-cached copy is otherwise immortal until its own recorded expiry.
// Only real hash-based thumb paths (matches the engine's own >71-char check
// used elsewhere to distinguish a real thumb from a short static
// placeholder like /genericThumb.png).
function versionThumb(thumb) {
  if (!thumb || thumb.length <= 71) { return thumb; }
  var v = (settings && settings.thumbAssetVersion) || 1;
  ...
  if (globalLatestImages) {
    ...
    for (i = 0; i < globalLatestImages.length; i++) {
      ...
      latestImages.push({
        ...
        postId : image.postId,
        thumb : versionThumb(image.thumb)
      });
...
exports.catalog = function(boardUri, threads, callback) {
  ...
  for (var i = 0; i < threads.length; i++) {
    ...
    if (thread.files && thread.files.length) {
      threadToPush.thumb = versionThumb(thread.files[0].thumb);
